...
parser.add_argument('--num_iterations', default=1000, type=int) # 10000
parser.add_argument('--num_epochs', default=10, type=int) # 200
...
def discriminator_step(
    ...
    (obs_traj, pred_traj_gt, obs_traj_rel, pred_traj_gt_rel, non_linear_ped,
     loss_mask, seq_start_end,demand_traj) = batch
    losses = {}
    loss = torch.zeros(1).to(pred_traj_gt)

    generator_out = generator(obs_traj, obs_traj_rel, seq_start_end,demand_traj)
    ...
    scores_fake = discriminator(traj_fake, traj_fake_rel, seq_start_end,demand_traj)
    scores_real = discriminator(traj_real, traj_real_rel, seq_start_end,demand_traj)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            (obs_traj, pred_traj_gt, obs_traj_rel, pred_traj_gt_rel,
             non_linear_ped, loss_mask, seq_start_end,demand_traj) = batch
            linear_ped = 1 - non_linear_ped
            loss_mask = loss_mask[:, args.obs_len:]

            pred_traj_fake_rel = generator(
                obs_traj, obs_traj_rel, seq_start_end,demand_traj
            )
            pred_traj_fake = relative_to_abs(pred_traj_fake_rel, obs_traj[-1])
