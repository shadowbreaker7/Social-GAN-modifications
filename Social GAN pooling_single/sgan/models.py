...
class Encoder(nn.Module):
    ...
    '''
    def init_hidden(self, batch,demand_traj):
        demand_traj = demand_traj.contiguous().view(1,-1).type(torch.float)
        # self.demand_embedding = nn.Linear(d.size(1),batch*self.h_dim)
        h = F.linear(demand_traj.cpu(),torch.Tensor(batch*self.h_dim,demand_traj.size(1)))
        h = h.reshape(self.num_layers,batch,self.h_dim).cuda()
        return (
    ...
    def forward(self, obs_traj,demand_traj):
        """
        Inputs:
...
class Decoder(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, state_tuple, seq_start_end):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
        ...
        pred_traj_fake_rel = []
        last_pos = obs_traj[-1]
        decoder_input = self.spatial_embedding(obs_traj_rel) #(8,batch,16)
        decoder_input = decoder_input.view(-1, batch, self.embedding_dim) #(8,batch,16)

        for _ in range(self.seq_len):
            output, state_tuple = self.decoder(decoder_input, state_tuple) # output(8,batch,32),state[0]/output[-1](1,batch,32)
            rel_pos = self.hidden2pos(output[-1].view(-1, self.h_dim)) #(batch,2)
            curr_pos = rel_pos + last_pos
            ...
            rel_pos_input = self.spatial_embedding(embedding_input)
            rel_pos_input = rel_pos_input.view(1,batch,-1)
            decoder_input = torch.cat((decoder_input[1:,:,:],rel_pos_input),0)
            pred_traj_fake_rel.append(rel_pos.view(batch, -1))
            last_pos = curr_pos
...
class PoolHiddenNet(nn.Module):
    ...
    def forward(self, h_states, seq_start_end, demand_traj):
        """
        Inputs:
        - h_states: Tensor of shape (num_layers, batch, h_dim)
        - seq_start_end: A list of tuples which delimit sequences within batch
        ...
        for _, (start, end) in enumerate(seq_start_end):
            ...
            start_ = int(start/2)
            end_ = int(end/2)
            demand_pos = demand_traj[0,start_:end_,:]
            # print(demand_pos.shape)
            demand_pos = demand_pos.reshape((-1,2))
            # print(demand_pos.shape)
            # Repeat -> H1, H2, H1, H2
            curr_hidden_1 = curr_hidden.repeat(num_ped, 1)
            # print(curr_hidden_1.shape)
            demand_embedding = self.spatial_embedding(demand_pos)
            # print(demand_embedding.shape)
            demand_embedding = demand_embedding.repeat(num_ped,1)
            demand_embedding = demand_embedding.repeat(num_ped,1)
            mlp_h_input = torch.cat([demand_embedding, curr_hidden_1], dim=1)
            curr_pool_h = self.mlp_pre_pool(mlp_h_input)
            curr_pool_h = curr_pool_h.view(num_ped, num_ped, -1).max(1)[0]
            pool_h.append(curr_pool_h)
        pool_h = torch.cat(pool_h, dim=0)
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, demand_traj,user_noise=None):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
        - obs_traj_rel: Tensor of shape (obs_len, batch, 2)
        ...
        # Encode seq
        final_encoder_h = self.encoder(obs_traj_rel,demand_traj)
        # Pool States
        if self.pooling_type:
            # end_pos = obs_traj[-1, :, :]
            pool_h = self.pool_net(final_encoder_h, seq_start_end, demand_traj)
...
class TrajectoryDiscriminator(nn.Module):
    ...
    def forward(self, traj, traj_rel, demand_traj,seq_start_end=None):
        """
        Inputs:
        ...
        """
        final_h = self.encoder(traj_rel,demand_traj)
