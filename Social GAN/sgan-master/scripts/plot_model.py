...
parser.add_argument('--model_path', default='../models/0',type=str)
...
num_t=0
fig, ax = plt.subplots()
ln11, = ax.plot([], [], 'b--',label = 'defender')
ln12, = ax.plot([], [], 'g--',label = 'attacker')
ln21, = ax.plot([], [], 'r--')
ln22, = ax.plot([], [], 'c--')
ln31, = ax.plot([], [], 'b:')
ln32, = ax.plot([], [], 'g:')
        ...
        for idx,batch in enumerate(loader):
            if idx == 2:
                ...
                for _ in range(1):
                    ...
                    gt = pred_traj_gt[:, start:end-1, :].data.cpu()
                    input_a = obs_traj[:, start:end-1, :].data.cpu()
                    out_a = pred_traj_fake[:, start:end-1, :].data.cpu()
